...
/**
 * Dar una etapa del programa por hecha, o desmarcarla.
 *
 * Lo que hoy se tilda a mano en la planilla. Es lo único que le da contra qué
 * comparar al calendario, así que es la marca más importante de la ficha.
 */
export async function marcarEtapaDelPrograma(
  clienteId: number, clave: string, hecha: boolean,
): Promise<{ ok: boolean; error?: string }> {
  const puede = await quienPuedeTocar(clienteId)
  if (!puede.ok) return { ok: false, error: puede.error }
